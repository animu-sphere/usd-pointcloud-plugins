...
Section 5 of the [design policy](../design/DESIGN_POLICY.md) requires a
...
[point reader architecture](POINT_READER.md). LOD generation consumes the
...
contracts. They appear only in contracts owned by `usdPointCloudAuthoring`.
...
## 10. `usdPointCloudAuthoring` Authoring

`libs/usd-pointcloud-authoring` owns the mapping from the shared contracts to the OpenUSD
...
in the [file-format argument contract](FILE_FORMAT_ARGUMENTS.md). The
...
Metadata-only reads are implemented for both LAS and LAZ. A metadata-only read
avoids decoding point records while still exposing source point count, source
bounds, CRS, scale and offset, point format, and available attributes. Tile
planning metadata and the default LOD policy are authored once spatial tiling
...
`usdPointCloudAuthoring` tests `UsdLodRootAPI` application, heuristic relationship targets,
...
output, and metadata-only behavior.
