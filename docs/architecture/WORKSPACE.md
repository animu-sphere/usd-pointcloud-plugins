...
| `usdLas` | `libs/usd-las` | plain CMake/OpenStrata static library | implemented | LAS parsing and decoding: 1.2-1.4 headers, VLR/EVLR, point formats 0-10, waveform packet metadata, GeoTIFF key records, scalar Extra Bytes, metadata-only reads, and chunked and range-based reads behind `LasReader`. |
...
| unreleased | typed diagnostics end to end, LAS 1.4 attributes and waveform formats, GeoTIFF key parsing, scalar Extra Bytes, chunked and range-based reads, file-format arguments, shared authoring entry point, `usdLod` authoring, compact LOD profiles, tiled and payload-backed authoring, metadata-only reads, and the module rename | on `main` |
