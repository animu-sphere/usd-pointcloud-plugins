...
| `usdCopc` | `libs/usd-copc` | plain CMake/OpenStrata static library | implemented (source-backed local read) | COPC Info and hierarchy validation, project-owned random-access source integration, selective point-data range decoding through the shared LAZ chunk decoder, and native hierarchy streaming. The OpenUSD `ArAsset` adapter and remote resolver path remain deferred. |
