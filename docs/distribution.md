...
included, and section 12 of the [development policy](development-policy.md).
...
| `laz-perf 2.0.0` | LGPL-2.1 | Vendored source, compiled into `geo-laz` |
...
`geo-las` contains no laz-perf code. Only `geo-laz` is affected by LGPL-2.1.
...
`geo-laz` plugin shared library.

```text
third_party/laz-perf/cpp/lazperf/*.cpp
  -> usdLaz (STATIC)
    -> geo-laz plugin (SHARED)
```

Consequences:

- The shipped `geo-laz` binary is a combined work that contains LGPL-2.1 object
...
Every distribution that includes `geo-laz` binaries provides:
...
5. The complete corresponding source for laz-perf, matching the exact version
   built.
6. A means for the recipient to relink `geo-laz` against a modified laz-perf.
...
rebuild `geo-laz` with their own laz-perf. Shipping the intermediate object
files or the static `usdLaz` archive is the fallback if a build from source
ever stops being reproducible.

Switching `geo-laz` to link laz-perf as a separate shared library would move
...
| Artifact | Purpose |
| --- | --- |
| Plugin product bundle | `geo-las` and `geo-laz` libraries and `plugInfo.json` |
...
| `supported-formats.md` | The format and attribute matrix for the release |
| `compatibility.md` | OpenUSD and OpenStrata compatibility statement |
...
notice, supported-formats, compatibility, and installation documents into the
release assets is outstanding work and blocks the first public binary release.
...
- [ ] The source archive builds `geo-laz` on every published platform.
- [ ] Checksums verify.
